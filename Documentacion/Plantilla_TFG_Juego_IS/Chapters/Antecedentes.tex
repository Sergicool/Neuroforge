...
Los primeros motores reutilizables surgieron en la década de 1990 a raíz del éxito de títulos como \textit{Doom} (id Software, 1993), cuyo motor fue licenciado a terceros y sentó las bases del concepto moderno de motor de videojuegos~\cite{Kushner2003}. A partir de entonces, la industria evolucionó hacia soluciones cada vez más genéricas y accesibles.

\textit{Unreal Engine}, lanzado por Epic Games en 1998, se consolidó como el motor de referencia en producciones de gran presupuesto y alta exigencia gráfica. Sus sistemas avanzados de renderizado, iluminación y simulación física permiten desarrollar entornos tridimensionales de alta fidelidad visual, lo que lo convierte en la herramienta estándar en la industria AAA. Esta potencia técnica tiene como contrapartida una curva de aprendizaje pronunciada, unos requisitos de hardware elevados y tiempos de compilación considerables, factores que pueden suponer una desventaja significativa en proyectos de pequeña escala o con recursos limitados~\cite{unrealengine_features}.

\textit{Unity}, publicado en 2005, se consolidó como el segundo gran referente del sector gracias a su versatilidad para desarrollar videojuegos en dos y tres dimensiones sobre una amplia variedad de plataformas, con una presencia especialmente destacada en el desarrollo independiente y móvil. Cuenta con una comunidad muy extensa, una gran cantidad de documentación y un ecosistema amplio de herramientas y recursos. Sin embargo, su arquitectura interna está históricamente orientada al desarrollo tridimensional, lo que puede introducir una sobrecarga innecesaria en proyectos 2D más sencillos. Además, la controversia generada en 2023~\cite{unity_runtimefee_apology, unity_runtimefee_cancelled} por una propuesta de tarificación por instalación del videojuego, ampliamente rechazada por la comunidad y finalmente revertida, puso en evidencia los riesgos asociados a depender de un motor de código cerrado con modelo de licencia comercial~\cite{unity_overview}.

En este contexto surge \textit{Godot Engine}, un motor de código abierto publicado bajo licencia MIT cuya primera versión estable se lanzó en 2014~\cite{GodotDocs}. Su arquitectura está basada en un sistema de nodos y escenas que facilita la organización modular del proyecto, y ofrece dos lenguajes de scripting nativos: \textit{GDScript}, con una sintaxis inspirada en Python orientada a la accesibilidad, y \textit{C\#}, integrado mediante el entorno .NET, que permite aprovechar el ecosistema .NET y resulta especialmente adecuado para proyectos que requieren mayor control sobre el rendimiento o integración con otras herramientas. Su motor 2D es un sistema nativo e independiente del motor 3D, lo que lo convierte en una herramienta especialmente eficiente para el desarrollo de videojuegos bidimensionales. Su editor ocupa menos de 200~MB, arranca en segundos y permite ciclos de iteración muy rápidos~\cite{godot_features}. A esto se suma la ausencia total de costes de licencia y la imposibilidad estructural de que las condiciones de uso cambien de forma unilateral al tratarse de software de código abierto.

\clearpage

Godot ha experimentado un crecimiento notable en los últimos años dentro del desarrollo independiente, en parte impulsado por la polémica de Unity de 2023. El caso más representativo es el de \textit{Slay the Spire~II} (MegaCrit, 2026): el estudio migró el proyecto íntegramente a Godot tras dicha polémica, asumiendo reescribir más de dos años de trabajo~\cite{slayspire2_godot}. El resultado fue uno de los lanzamientos independientes más exitosos de los últimos años, alcanzando un pico de más de 500.000 jugadores simultáneos en Steam en sus primeros tres días en Early Access y superando los tres millones de unidades vendidas en su primera semana~\cite{slayspire2_launch}. Este caso ilustra que Godot es capaz de sustentar productos comerciales de alto nivel cuando el tipo de videojuego se alinea con sus fortalezas.

La Tabla~\ref{tab:comparativa_motores} resume las principales diferencias entre los tres motores.
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Inteligencia artificial en videojuegos de estrategia}
% ─────────────────────────────────────────────────────────────────────────────
...
Los primeros enfoques consolidados se basaron en algoritmos de búsqueda en árbol. El algoritmo \textit{Minimax}, formalizado en la década de 1950 a partir de los trabajos de John von Neumann sobre teoría de juegos \cite{vonNeumann1944}, evalúa el árbol de posibles estados del videojuego asumiendo que ambos jugadores actúan de forma óptima: el agente maximiza su propio beneficio mientras el rival lo minimiza. Su variante \textit{Alpha-Beta Pruning}, propuesta por John McCarthy y desarrollada por otros investigadores durante los años 1960 \cite{Knuth1975}, mejora considerablemente la eficiencia al descartar ramas del árbol que no pueden influir en la decisión final. Estos algoritmos demostraron gran efectividad en videojuegos de información completa, siendo la base de programas históricos como \textit{Deep Blue} \cite{Campbell2002}. Sin embargo, en videojuegos con información oculta como \textit{Stratego}, el espacio de estados se vuelve exponencialmente mayor al desconocer la disposición enemiga, lo que limita su aplicabilidad práctica.
